    ...
    <div className="auth" style={{height: window.innerHeight - 32}}>
      <form className='auth-form'>
        <div style={{textAlign: 'center'}}>
          <h2 style={{fontSize: '24px'}}>{isLogin ? 'Авторизация' : 'Регистрация'}</h2>
        </div>
        <input placeholder='Email' value={email} onChange={(e) => setEmail(e.target.value)}/>
        <input placeholder='Password' value={password} onChange={(e) => setPassword(e.target.value)} type={'password'}/>
        <div className='auth-buttons'>
          {isLogin && (<Link to='/registration'>Зарегистрироваться</Link>)}
          {!isLogin && (<Link to='/login'>Войти в существующий</Link>)}
